    ...
    private function insertSubjectsForClass($class, $subjects, $now)
    {
        foreach ($subjects as $sub) {
            ...
            DB::table('subject__takens')->insert([
                'student_id' => NULL,'classroom_id' => $class->id,'subject_id' => $sub->id,'subject_teacher_id' => $subsTeacher->isNotEmpty() ? $subsTeacher->random() : NULL,'created_at' => $now,'updated_at' => $now
            ]);
        }
    }
}
